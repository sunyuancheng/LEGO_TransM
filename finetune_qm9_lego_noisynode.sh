...
[ -z "${save_path}" ] && save_path='./logs/finetune_qm9_LEGO'
...
[ -z "${save_prefix}" ] && save_prefix='noisynodes'
...
python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $ddp_options train.py \
	...
	--task graph_prediction_qm9 --criterion graph_prediction_qm9_noisynodes --arch transformer_m --load-qm9 --remove-head --num-classes 1 \
